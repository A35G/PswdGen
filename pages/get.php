<?php
  ...
  function randPass ( $upper = 3, $lower = 3, $numeric = 3, $other = 3 ) {
    ...
    for ( $i = 0; $i < $upper; $i++ ) { $passOrder[] = chr ( rand ( 65, 90 ) ); }
    ...
    for ( $i = 0; $i < $numeric; $i++) { $passOrder[] = chr ( rand ( 48, 57 ) ); }
	  ...
	  foreach ( $passOrder as $char ) { $passWord .= $char; }
  ...
  if ( isset ( $_GET['tip_r'] ) && !empty ( $_GET['tip_r'] ) ) {
    ...
    switch ( $num ) {                      
      case '1': $tips = str_replace ( "1", "12", $tips ); break;
      case '2': $tips = str_replace ( "1", "6", $tips ); break;
      case '3': $tips = str_replace ( "1", "4", $tips ); break;
      case '4': $tips = str_replace ( "1", "3", $tips ); break;    
    }
